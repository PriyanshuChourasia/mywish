                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="date" name="start_date" readonly class="form-control" id="" value="<?php echo date('Y-m-d') ?>">
                        </div>
                    </div>
                    <div class="col-md-6">
                        <label for="">Status</label>
                        <input type="text" class="form-control" readonly value="active" name="status">
                    </div>
                </div>
                <div class="text-center mt-4">
                    <input type="submit" value="{{__('Confirm Course')}}" class="btn btn-success" id="status_register">
                </div>
            </form>
        </div>
...
<script>
    $(document).ready(function(){
        $('#register_status').on('submit', function(e){
            ...
            $.ajax({
                url:"{{route('admin.save_status',$students->id)}}",
                type:"POST",
                data:formData,
                contentType: false,
                processData: false,
